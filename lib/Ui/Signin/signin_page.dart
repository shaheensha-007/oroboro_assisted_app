...
final username1 = GlobalKey<FormState>();
...
class _Signin_pageState extends State<Signin_page> {
  ...
  listener: (context, state) {
    ...
    if(state is TokenblocLoaded){
      tokenModel = BlocProvider
          .of<TokenBloc>(context).tokenModel;
  ...
  listener: (context, state)async {
    ...
    if(state is SigninblocLoaded){
       ...
       String success=isSigninsucess.result!.status.toString();
      if(success=="SUCCESS"){
        await preferences.setString('success', success);
        Navigator.of(context).pushAndRemoveUntil(
            MaterialPageRoute(
                builder: (context) => const Mainhome()),
                (route) => false);
      }else{
       String messagedata=isSigninsucess.message.toString();
        _showErrorSnackBar(messagedata);
      }
    }
    // TODO: implement listener
  },
  child: ElevatedButton(
                        ...
                        onPressed: () async{
                       BlocProvider.of<SigninBloc>(context).add(FetchSignin(userName: Signinusername.text, password: Signinpassword.text, ctx: context));
                        },
                        child: const Text(
                          "Sign in",
                          style: TextStyle(
                    ...
                    listener: (context, state) {
                      ...
                      if (state is TokenblocLoaded) {
                        ...
                        if (isvalid == true) {
                          ...
                          Navigator.of(context).pushAndRemoveUntil(
                              MaterialPageRoute(
                                  builder: (context) => const Agent_onborading()),
                              (route) => false);
                        }
                      }
                      // TODO: implement listener
